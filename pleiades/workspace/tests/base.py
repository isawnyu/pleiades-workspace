from pleiades.workspace.content.workspace import initStateTopic
...
class WorkspaceFunctionalTestCase(ptc.FunctionalTestCase):
    ...
    def afterSetUp(self):
        ...
        mid = self.portal['features']['metadata'].invokeFactory(
            'PositionalAccuracy', id='cap-map65')
        self.portal['features']['metadata'][mid].setValue(0.01)
        self.portal['features']['metadata'][mid].setText(
            "That's right, 1 cm!")
...
class ContentFunctionalTestCase(ptc.FunctionalTestCase):
    ...
    def afterSetUp(test):
        ...
        mid = test.portal['features']['metadata'].invokeFactory(
            'PositionalAccuracy', id='cap-map65')
        ...
        # Add feature
        fid = test.features.invokeFactory(
            'Feature', '1', title='Ninoe', featureType='settlement')
        f = test.features[fid]
        ...
        f.invokeFactory(
            'Location', 'location', title='Point 1',
        ...
        # Add a workspace
        wsid = test.workspaces.invokeFactory(
            'Workspace', 'test-ws', title='Test Workspace')
